"""Build the 24-channel (19 + neighbourhood-ensemble) 20yr feature cache, X only.

Copy of build_cache_parallel.py using build_nbhd_sequences; labels are reused from the
19ch cache (same 7274 dates, same order), so this writes X.dat + standardizer only.
...
from src.features.build_gridded_sequences_nbhd import (
    NBHD_N_CHANNELS as N_CHANNELS, build_nbhd_sequences as build_gridded_sequences, standardize_nbhd,
)
...
def build_shard(job: tuple) -> tuple[int, int, int]:
    ...
    for offset, path in enumerate(files):
        with xr.open_dataset(path) as ds:
            ...
            features[start + offset] = standardize_nbhd(seq, mean, std)[0]
    features.flush()
...
def main() -> None:
    ...
    t0 = time.time()
    std_path = cache_dir / "standardizer.npz"
    if std_path.exists():  # a restart reuses the (deterministic) fit instead of redoing it
        data = np.load(std_path)
        standardizer = ChannelStandardizer(mean=data["mean"], std=data["std"])
        print(f"reusing {std_path}; allocating memmaps ...", flush=True)
    else:
        standardizer = fit_standardizer(files[:n_train])
        np.savez(std_path, mean=standardizer.mean, std=standardizer.std)
        print(f"standardizer fitted in {time.time() - t0:.0f}s (saved); allocating memmaps ...", flush=True)
